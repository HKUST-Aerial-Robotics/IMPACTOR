 ...
 * Description: This is the source file for the AStar class, which is
 *              used to find the path in the 3D space. Modified from the
 *              original code in project ego-planner
...
bool AStar::AstarSearch(const double step_size, Vector3d start_pt, Vector3d end_pt, bool use_esdf_check)
{
    ...
    while (!openSet_.empty())
    {
        ...
        ros::Time time_2 = ros::Time::now();
        if ((time_2 - time_1).toSec() > 100.0)
        {
            ROS_WARN("Failed in A star path searching !!! 10.0 seconds time limit exceeded.\n");
            return false;
        }
    }
...
vector<Vector3d> AStar::astarSearchAndGetSimplePath(const double step_size, Vector3d start_pt, Vector3d end_pt, bool use_esdf_check)
{
    // call astar search and get the path
    AstarSearch(step_size, start_pt, end_pt, use_esdf_check);
